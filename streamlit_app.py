...
# --- ΚΥΡΙΩΣ ΠΑΙΧΝΙΔΙ ---
    ...
    if st.session_state.memory_mode:
        time_passed = int(time.time() - st.session_state.memory_start)
        time_left = 20 - time_passed
        if time_left <= 0:
            st.session_state.memory_mode = False
            st.session_state.start_time = time.time()
            st.rerun()
        msg = f"👀 Απομνημόνευσε! Κλείνουν σε: {max(0, time_left)}"
        st.markdown(f'<div class="countdown-box">{msg}</div>', unsafe_allow_html=True)
    
    # Stats Header (Μόνο μετά το Memory Mode)
    if not st.session_state.memory_mode:
        ...
        # Placeholder για να μην μετακινείται το πλέγμα
        st.write("") 
    ...
    for row in range(3):
        ...
        for col in range(4):
            ...
            # Εμφάνιση περιεχομένου αν είμαστε σε Memory Mode ή αν η κάρτα είναι ανοιχτή/ταιριασμένη
            ...
            with cols[col]:
                ...
                # Κουμπί ΚΛΙΚ μόνο αν δεν είναι Memory Mode και η κάρτα είναι κλειστή
                if not st.session_state.memory_mode and not (is_f or is_m):
                    if st.button("ΚΛΙΚ", key=f"b{idx}", use_container_width=True):
    ...
    if not st.session_state.memory_mode:
        if len(st.session_state.flipped_indices) == 2:
            ...
            if st.session_state.deck[i1]['value'] == st.session_state.deck[i2]['value'] and st.session_state.deck[i1]['type'] != st.session_state.deck[i2]['type']:
                ...
                st.rerun() # Refresh για να δει ο χρήστης τη δεύτερη κάρτα
        
        # Αν έχουμε 2 κάρτες και δεν ταιριάζουν, περιμένουμε λίγο και τις κλείνουμε
        if len(st.session_state.flipped_indices) == 2:
             time.sleep(1.0)
             st.session_state.flipped_indices = []
             st.rerun()
        ...
        # Αυτόματο refresh για το countdown
        time.sleep(0.1)
        st.rerun()
