...
<style>
    ...
    /* Σταθερό ύψος για τις κάρτες ώστε να μην αλλάζουν θέση */
    .card-container {
        height: 160px; /* 110px κάρτα + 50px για το κουμπί/κενό */
        display: flex;
    ...
    /* Μπλε Κουμπιά Flat - Χωρίς περιγράμματα */
    ...
    with placeholder.container():
        ...
        if selected:
            if st.button("🚀 ΞΕΚΙΝΑΜΕ!", type="primary", use_container_width=True):
                ...
                placeholder.empty() # Καθαρίζει τα πάντα
                st.rerun()
        else:
            ...
            for col in range(4):
                ...
                with grid_cols[col]:
                    ...
                    if not st.session_state.memory_mode and not (is_f or is_m):
                        if st.button("ΚΛΙΚ", key=f"play_{idx}", use_container_width=True):
